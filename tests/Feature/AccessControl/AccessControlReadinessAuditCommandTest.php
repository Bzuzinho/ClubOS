...
class AccessControlReadinessAuditCommandTest extends TestCase
{
    ...
    public function test_audit_reports_unresolved_platform_access_users_and_route_guard_gaps_without_writing(): void
    {
        ...
        $unresolved = User::factory()->create(['perfil' => null]);
        ...
        $this->assertGreaterThan(0, $payload['summary']['modules_without_granular_permission_tree_count']);
        ...
        $this->assertTrue($codes->contains('menu_module_without_granular_permission_tree'));
    ...
    {
        $user = User::factory()->create(['perfil' => null]);
        DadosConfiguracao::query()->create([
            'user_id' => $user->id,
            'platform_access_enabled' => true,
        ]);
