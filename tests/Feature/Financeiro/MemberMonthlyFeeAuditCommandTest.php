...
final class MemberMonthlyFeeAuditCommandTest extends TestCase
{
    ...
    public function test_command_covers_classifications_and_json_payload_contract(): void
    {
        ...
        $canonicalOnly = User::factory()->create(['estado' => 'ativo', 'ativo_desportivo' => true]);
        ...
        $missingRequired = User::factory()->create(['estado' => 'ativo', 'ativo_desportivo' => true]);
        $missingRequired->userTypes()->sync([$athleteType->id]);

        $notRequired = User::factory()->admin()->create(['estado' => 'ativo', 'tipo_membro' => [], 'ativo_desportivo' => false]);
        ...
        $this->assertSame(3, (int) ($summary['total_users'] ?? 0));
        $this->assertSame(1, (int) ($summary['canonical_only_count'] ?? 0));
        $this->assertSame(0, (int) ($summary['matching_count'] ?? 0));
        $this->assertSame(0, (int) ($summary['legacy_fallback_count'] ?? 0));
        $this->assertSame(0, (int) ($summary['divergent_count'] ?? 0));
        ...
        $this->assertSame(0, (int) ($summary['invalid_reference_count'] ?? 0));
    ...
    public function test_report_path_writes_payload_file(): void
    {
        ...
        $athleteType = $this->createUserType('atleta', 'Atleta');
        $required = User::factory()->create(['estado' => 'ativo', 'ativo_desportivo' => true]);
    ...
    public function test_user_option_limits_scope(): void
    {
        ...
        $target = User::factory()->create(['estado' => 'ativo', 'ativo_desportivo' => true]);
        $target->userTypes()->sync([$athleteType->id]);
        DadosFinanceiros::query()->create(['user_id' => $target->id, 'mensalidade_id' => $plan->id]);

        $extra = User::factory()->create(['estado' => 'ativo', 'ativo_desportivo' => true]);
    ...
    public function test_fail_on_divergence_returns_zero_without_legacy_column(): void
    {
        $plan = $this->createPlan('Plano Divergence Disabled');
        $athleteType = $this->createUserType('atleta', 'Atleta');
        $user = User::factory()->create(['estado' => 'ativo', 'ativo_desportivo' => true]);
    ...
    public function test_fail_on_fallback_returns_zero_without_legacy_column(): void
    {
        $plan = $this->createPlan('Plano Fallback Disabled');
        $athleteType = $this->createUserType('atleta', 'Atleta');
        $user = User::factory()->create(['estado' => 'ativo', 'ativo_desportivo' => true]);
        $user->userTypes()->sync([$athleteType->id]);
        DadosFinanceiros::query()->create(['user_id' => $user->id, 'mensalidade_id' => $plan->id]);
