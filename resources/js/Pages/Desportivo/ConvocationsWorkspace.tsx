...
export default function ConvocationsWorkspace({ convocations, events, athletes, cost_centers, race_catalog }: Props) {
  ...
  return <AuthenticatedLayout fullWidth>
    ...
    <div className="w-full min-w-0 space-y-3">
      ...
      <div className="flex flex-wrap gap-1">{([['convocations', 'Convocatórias'], ['responses', 'Respostas'], ['logistics', 'Logística'], ['history', 'Histórico']] as const).map(([key, label]) => <Button key={key} size="sm" variant={view === key ? 'default' : 'outline'} onClick={() => setView(key)}>{label}</Button>)}</div>
    ...
    <Dialog open={detailOpen} onOpenChange={setDetailOpen}><DialogContent className="w-[calc(100vw-1rem)] max-w-5xl overflow-x-hidden p-3 sm:w-[calc(100vw-2rem)] sm:p-6"><DialogHeader><DialogTitle>Consultar Convocatória · {detail?.event.title}</DialogTitle></DialogHeader>{detail && <div className="space-y-3"><div className="grid grid-cols-2 gap-2 md:grid-cols-5"><K n={detail.stats.total} l="Convocados" /><K n={detail.stats.confirmed} l="Confirmados" /><K n={detail.stats.pending} l="Pendentes" /><K n={detail.stats.declined} l="Recusaram" /><K n={eur(detail.cost.calculated_value)} l="Custo" /></div><div className="flex flex-wrap gap-1">{([['summary', 'Resumo'], ['athletes', 'Atletas e provas'], ['responses', 'Respostas'], ['logistics', 'Logística'], ['publication', 'Publicação'], ['costs', 'Custos']] as const).map(([key, label]) => <Button key={key} size="sm" variant={detailTab === key ? 'default' : 'outline'} onClick={() => setDetailTab(key)}>{label}</Button>)}</div><DetailBody tab={detailTab} d={detail} races={race_catalog} /></div>}<DialogFooter><Button variant="outline" onClick={() => setDetailOpen(false)}>Fechar</Button>{detail && <Button onClick={() => void publish(detail.id)}>{detail.publication_status === 'published' ? 'Republicar' : 'Publicar'}</Button>}</DialogFooter></DialogContent></Dialog>

    <Dialog open={createOpen} onOpenChange={setCreateOpen}><DialogContent className="w-[calc(100vw-1rem)] max-w-5xl overflow-x-hidden p-3 sm:w-[calc(100vw-2rem)] sm:p-6"><DialogHeader><DialogTitle>Nova Convocatória · Passo {step} de 5 · {stepTitle}</DialogTitle></DialogHeader><div className="grid min-w-0 gap-4 md:grid-cols-[170px_minmax(0,1fr)]"><div className="space-y-1">{['Evento', 'Atletas', 'Provas / estafetas', 'Logística / custos', 'Rever / publicar'].map((label, index) => <div key={label} className={`rounded-md px-3 py-2 text-xs ${step === index + 1 ? 'bg-sky-50 font-semibold text-sky-800' : ''}`}>{index + 1}. {label}</div>)}</div><div className="min-w-0 max-h-[65vh] overflow-y-auto overflow-x-hidden pr-1"><WizardStep step={step} draft={draft} setDraft={setDraft} events={events} athletes={athletes} selectedEvent={selectedEvent} selectedIds={selectedIds} toggleAthlete={toggleAthlete} updateAthlete={updateAthlete} costCenters={cost_centers} races={race_catalog} /></div></div><DialogFooter className="gap-1"><Button variant="outline" disabled={step === 1} onClick={() => setStep(value => Math.max(1, value - 1))}>Anterior</Button>{step < 5 ? <Button onClick={() => setStep(value => Math.min(5, value + 1))}>Seguinte</Button> : <Button disabled={saving || !draft.event_id || draft.athletes.length === 0} onClick={() => void save()}>{saving ? 'A criar…' : 'Criar convocatória'}</Button>}</DialogFooter></DialogContent></Dialog>
  </AuthenticatedLayout>;
}
