...
function ConfigurationDialog({
    ...
    const initial = useMemo(() => Object.fromEntries(
        catalog.fields.map((field) => [field.key, row?.[field.key] ?? emptyValueFor(field)])
    ), [catalog, row]);
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>{row ? 'Editar' : 'Nova configuração'} — {catalog.label}</DialogTitle>
                </DialogHeader>
                            ...
                            return (
                                <div key={field.key} className="flex items-center justify-between gap-4 rounded-lg border p-3 sm:col-span-2">
                                    ...
                                    <Switch
                                        ...
                                        onCheckedChange={(checked) => setData((current) => ({ ...current, [field.key]: checked }))}
                                    />
                                </div>
                            );
                        }
...
function CatalogPanel({ catalog, rows }: { catalog: CatalogDefinition; rows: ConfigRow[] }) {
    ...
    const remove = (row: ConfigRow) => {
        ...
        router.delete(route('desportivo.configuracao.destroy', { catalog: catalog.key, id: row.id }), {
            preserveScroll: true,
        });
    };
        ...
        <Card>
            <CardHeader className="gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    ...
                    <CardDescription className="mt-1 max-w-3xl">{catalog.description}</CardDescription>
                </div>
            ...
            <CardContent>
                <div className="overflow-x-auto rounded-lg border">
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {rows.length === 0 ? (
                                <tr><td colSpan={5} className="px-3 py-8 text-center text-muted-foreground">Sem registos.</td></tr>
                            ) : rows.map((row) => (
                                <tr key={row.id} className="border-t">
                                    <td className="px-3 py-2 font-mono text-xs">{row.codigo}</td>
                                    <td className="px-3 py-2 font-medium">{row.nome}</td>
                                    <td className="px-3 py-2">
                                        <Badge variant={row.archived || !row.ativo ? 'secondary' : 'default'}>
                                            {row.archived ? 'Arquivado' : row.ativo ? 'Ativo' : 'Inativo'}
                                        </Badge>
                                    </td>
                                    <td className="px-3 py-2 text-muted-foreground">{row.used ? 'Em uso' : 'Sem referências'}</td>
                                    <td className="px-3 py-2">
                                        <div className="flex justify-end gap-1">
                                            <Button variant="ghost" size="sm" onClick={() => openEdit(row)} aria-label={`Editar ${row.nome}`}>
                                                <PencilSimple size={15} />
                                            </Button>
                                            <Button variant="ghost" size="sm" onClick={() => remove(row)} aria-label={`${row.used ? 'Arquivar' : 'Eliminar'} ${row.nome}`}>
                                                <Trash size={15} />
                                            </Button>
                                        </div>
                                    </td>
            ...
            {dialogOpen && (
                <ConfigurationDialog
                    key={`${catalog.key}-${editing?.id ?? 'new'}`}
                    catalog={catalog}
                    row={editing}
                    open={dialogOpen}
...
export default function SportsConfigurationIndex({
    ...
    race_types = [],
    limitation_types = [],
    legacy_injury_reasons = [],
}: Props) {
    const data: Record<string, ConfigRow[]> = {
        athlete_statuses,
        training_types,
        training_zones,
        absence_reasons,
                ...
                <div className="flex w-full items-center justify-between gap-4">
                    <div>
                        <h1 className="flex items-center gap-2 text-lg font-semibold tracking-tight sm:text-xl">
                            <GearSix size={20} /> Configuração Desportiva
                        </h1>
                        <p className="mt-0.5 text-xs text-muted-foreground">Catálogos técnicos e regras operacionais do módulo Desportivo</p>
                    </div>
                    <Button variant="outline" size="sm" className="gap-2" onClick={() => router.get(route('desportivo.index'))}>
                        <ArrowLeft size={15} /> Desportivo
                    </Button>
                </div>
            }
        >
            ...
            <div className="mx-auto w-full max-w-[1600px] space-y-4 p-3 sm:p-4">
                <Card className="border-blue-200 bg-blue-50/50">
                    <CardContent className="py-3 text-sm text-muted-foreground">
                        Os códigos técnicos podem ser alterados enquanto não tiverem histórico. Depois de usados ficam imutáveis; ao remover uma definição usada, o sistema arquiva-a em vez de destruir referências antigas.
                    </CardContent>
                </Card>
                ...
                <Tabs defaultValue="athlete_statuses" className="space-y-4">
                    <TabsList className="h-auto w-full justify-start gap-1 overflow-x-auto p-1">
                        {CATALOGS.map((catalog) => (
                            <TabsTrigger key={catalog.key} value={catalog.key} className="whitespace-nowrap text-xs">
                                {catalog.label}
                            </TabsTrigger>
                        ))}
                    </TabsList>
                    ...
                    {CATALOGS.map((catalog) => (
                        <TabsContent key={catalog.key} value={catalog.key}>
                            ...
                            {catalog.key === 'limitation_types' && legacy_injury_reasons.length > 0 && (
                                <Card className="mt-4">
                                    <CardHeader>
                                        <CardTitle className="text-sm">Catálogo clínico legacy — apenas leitura</CardTitle>
                                        <CardDescription>
                                            Estes motivos de lesão são preservados para compatibilidade histórica. Não são apresentados ao treinador como limitações operacionais.
                                        </CardDescription>
                                    </CardHeader>
                                    <CardContent className="space-y-2">
                                        {legacy_injury_reasons.map((item) => (
                                            <div key={item.id} className="flex items-center justify-between rounded-lg border px-3 py-2 text-sm">
                                                <div>
                                                    <span className="font-medium">{item.nome}</span>
                                                    <span className="ml-2 font-mono text-xs text-muted-foreground">{item.codigo}</span>
                                                </div>
                                                <Badge variant="secondary">{item.gravidade}</Badge>
                                            </div>
                                        ))}
                                    </CardContent>
                                </Card>
