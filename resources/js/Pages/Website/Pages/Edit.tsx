...
function FramePreview({ width, children }: { width: number; children: ReactNode }) {
    ...
    const scale = Math.min(1, Math.max(1, bounds.width) / width);
    ...
    return <div ref={container} className="relative h-full w-full min-w-0 overflow-hidden"><iframe ref={frame} title="Pré-visualização em tempo real" onLoad={initialise} style={{ width, height: Math.max(1, bounds.height) / scale, transform: `scale(${scale})`, transformOrigin: 'top left', left: Math.max(0, (bounds.width - width * scale) / 2) }} className="absolute top-0 max-w-none rounded-lg border-0 bg-white shadow-2xl" srcDoc="<!doctype html><html><head></head><body></body></html>" />{target && createPortal(children, target)}</div>;
}
...
export default function WebsitePageEdit({ page, media, blockTypes, news = [], events = [], partners = [], dynamicData, dataSources = DEFAULT_DATA_SOURCES }: { page: Page; media: MediaItem[]; blockTypes: BlockType[]; news?: PublicNews[]; events?: PublicEvent[]; partners?: PublicPartner[]; dynamicData?: WebsiteDynamicData; dataSources?: WebsiteDataSource[] }) {
    ...
    const [device, setDevice] = useState<Device>(() => typeof window !== 'undefined' && window.matchMedia('(max-width: 767px)').matches ? 'mobile' : 'desktop');
    ...
    return <AuthenticatedLayout fullWidth><div data-testid="website-editor" className="flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-y-auto xl:overflow-hidden bg-slate-100 text-foreground">
        <Head title={`${data.title} — Editor visual`} />
        <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 py-2 border-b bg-white px-3 shadow-sm lg:px-4">
            ...
            <div className="flex items-center gap-1 rounded-lg border bg-slate-50 p-1">{(['desktop', 'tablet', 'mobile'] as Device[]).map((item) => <Button key={item} variant={device === item ? 'default' : 'ghost'} size="icon" className="h-8 w-8" onClick={() => setDevice(item)} title={item}>{item === 'desktop' ? <Desktop /> : item === 'tablet' ? <DeviceTablet /> : <DeviceMobile />}</Button>)}</div>
            <div className="flex flex-wrap items-center gap-2">{page.can_import_current_website && <Button variant="outline" size="sm" className="inline-flex" onClick={importCurrentWebsite} disabled={saving}><DownloadSimple className="mr-2" /> Importar website atual</Button>}<Button asChild variant="outline" size="sm" className="inline-flex"><a href={`/website/paginas/${page.id}/previsualizar`} target="_blank" rel="noreferrer"><Eye className="mr-2" /> Pré-visualizar</a></Button><Button variant="outline" size="sm" onClick={() => submit('save_draft')} disabled={saving}><FloppyDisk className="mr-2" /> Guardar</Button><Button size="sm" onClick={() => submit('publish')} disabled={saving}>Aplicar</Button><Button variant="ghost" size="icon" onClick={close}><X /></Button></div>
        </header>
        {saveError && <div className="shrink-0 bg-red-600 px-4 py-2 text-center text-xs font-medium text-white">{saveError}</div>}

        <div className="grid min-w-0 shrink-0 grid-cols-1 xl:min-h-0 xl:flex-1 xl:grid-cols-[260px_minmax(0,1fr)_350px]">
            <aside aria-label="Estrutura da página" className="flex h-80 min-h-0 min-w-0 flex-col border-r bg-white xl:h-auto">
            ...
            <section aria-label="Pré-visualização da página" className="h-[65dvh] min-h-80 min-w-0 bg-[radial-gradient(circle_at_top,#dce7ef,#aebdca)] p-3 xl:h-auto xl:min-h-0 xl:p-5">
                <div className="mx-auto h-full w-full min-w-0"><FramePreview width={deviceWidths[device]}><ManagedPageCanvas page={livePage} news={news} events={events} partners={partners} dynamicData={resolvedDynamicData} editor={selectBlock} selectedBlockKey={selectedKey} selectedItemId={selectedItemId} /></FramePreview></div>
            </section>

            <aside aria-label="Propriedades da página" className="h-[75dvh] min-h-96 min-w-0 border-l bg-white xl:h-auto xl:min-h-0">
                <Tabs value={inspectorTab} onValueChange={(value) => setInspectorTab(value as InspectorTab)} className="flex h-full flex-col">
                    <div className="shrink-0 space-y-2 border-b p-3"><div><p className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Elemento selecionado</p><ModuleTabsList label="Ferramentas do editor">{selectedBlock ? <><TabsTrigger value="content">Conteúdo</TabsTrigger><TabsTrigger value="design">Estilo</TabsTrigger><TabsTrigger value="behavior">Comportamento</TabsTrigger></> : <span className="px-2 py-1 text-xs text-muted-foreground">Seleciona um elemento na página</span>}</ModuleTabsList></div><div><p className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Ferramentas da página</p><ModuleTabsList label="Ferramentas do editor"><TabsTrigger value="page">Página</TabsTrigger><TabsTrigger value="media">Imagens</TabsTrigger><TabsTrigger value="history">Histórico</TabsTrigger></ModuleTabsList></div></div>
