...
### H6c — Adapters e webhooks autenticados dos providers — implementado, promoção produtiva pendente
...
Próximo passo: promover H6c, confirmar o schema/eventos no audit produtivo e depois avançar para H6d (Redes como provider do mesmo pipeline). A ativação externa de callbacks exige secrets configurados no ClubOS e no fornecedor. Em paralelo, a campanha legacy agendada/vencida deve ser revista explicitamente; o sistema não a enviará sozinho. Um eventual cutover da queue produtiva `database` para Redis só deve ocorrer depois de validar a configuração efetiva do Supervisor. H3, H4, H5, H2.5, stock por variante e Família/EE estão estruturalmente fechados. A fila fiscal produtiva e a ação operacional Cloudflare R2 permanecem pendências operacionais separadas.
...
|---|---|---|---|---|
| 2026-09-03 | Comunicação / Segurança / Infraestrutura | H6c extrai adapters de email/SMS/push, exige referências externas rastreáveis e introduz callbacks HMAC anti-replay com eventos minimizados, idempotentes e transições monotónicas. | `CommunicationChannelAdapter`; adapters; `CommunicationProviderEventService`; `VerifyCommunicationWebhookSignature`; `communication_provider_events`; audit v3; testes funcionais e de arquitetura | Implementado; promoção PR/CI/deploy e audit produtivo pendentes. Ativação de providers/webhooks depende de credenciais reais. Comunicação 75% → 80%. |
