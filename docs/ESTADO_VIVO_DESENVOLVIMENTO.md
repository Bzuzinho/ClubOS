...
### H6b — Cutover assíncrono das automações — implementado, promoção produtiva pendente
...
Próximo passo: promover H6b com CI verde, validar no audit produtivo que não existe outbox automática por recuperar e depois avançar para H6c (adapters/providers e webhooks autenticados). Em paralelo, a campanha legacy agendada/vencida deve ser revista explicitamente; o sistema não a enviará sozinho. Um eventual cutover da queue produtiva `database` para Redis só deve ocorrer depois de validar a configuração efetiva do Supervisor. H3, H4, H5, H2.5, stock por variante e Família/EE estão estruturalmente fechados. A fila fiscal produtiva e a ação operacional Cloudflare R2 permanecem pendências operacionais separadas.
...
|---|---|---|---|---|
| 2026-09-03 | Comunicação / Financeiro / Eventos / Logística / Desportivo | H6b substitui entregas inline das automações e do gateway Desportivo por campanha-outbox + job após commit, preserva preferências/origens/idempotência e recupera campanhas automáticas sem entrega. | `CommunicationCampaignService::queueIndividualCommunication`; `CommunicationAutomationService`; `SportsCommunicationGatewayService`; `DispatchDueCommunicationsCommand`; audit v2; testes de automação/pipeline/contracts | Implementado; promoção PR/CI/deploy e evidência produtiva pendentes. Comunicação 70% → 75%. |
