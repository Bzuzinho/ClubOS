...
| Financeiro geral | 90% | Maduro; CRUDs legacy de transações/categorias aposentados e antigo `Financeiro/Edit` converge para o fluxo canónico. H5b integra também a Loja em `Invoice` + `PaymentAllocationService`, sem `Movement` paralelo. H1.18 garante entrada pelo menu em desktop/mobile; prioridade: preservar invariantes e acrescentar E2E financeiro crítico. |
| Fiscal | 89% | H4 fechado em produção com `manual_wintouch`: emissão externa e confirmação no ClubOS, sem chamada DLL/.NET pelo Laravel/Linux. H5c acrescenta a projeção fiscal da Loja e prova pagamento confirmado → pedido Wintouch → registo do documento externo. O processamento automático fica fail-closed sem `provider_api`; a fila operacional mantém tratamento separado. |
| Inventário / Logística | 74% | `stock_movements` é o ledger canónico, H2.4b integrou variantes atomicamente e H5a compensa no ledger as saídas de encomendas canceladas, incluindo variante. H5b prova que a fatura da Loja não cria uma segunda saída física e mede o contrato em produção. Falta fechar compras/requisições/empréstimos transversalmente e ampliar QA operacional. |
| Loja | 78% | H5a fecha cancelamento/reposição pré-entrega, H5b cria a `Invoice` canónica e H5c projeta pagamento, saldo e fiscalidade diretamente dessa fatura na encomenda, sem coluna financeira duplicada nem alterar automaticamente o estado logístico. Permanece a devolução/reversão posterior à entrega e o lifecycle interno de Logística. |
