...
|---|---:|---|
| Base técnica / arquitetura | 89% | H0.1a/H0.1b/H0.2 concluídos em produção. H1.1 concluiu dependency remediation compatível e QA ratchets; H1.4/H1.5/H1.6/H1.7 reduziram progressivamente a dívida TypeScript para 66 erros/27 ficheiros. Permanecem Laravel major, migração de `xlsx`, continuação do paydown TypeScript e hardenings residuais. |
...
| PWA / Mobile | 55% | H1.4/H1.5/H1.6/H1.7 reduziram a dívida TypeScript de 132 erros/55 ficheiros para 66 erros/27 ficheiros e apertaram o ratchet anti-regressão para esse teto. Continuam em falta lint, unit/component tests, E2E, acessibilidade e matriz Android/iOS/tablet/desktop. |
...
- TypeScript começou com teto de 132 erros / 55 ficheiros; qualquer melhoria tem de baixar o baseline no mesmo PR. H1.4 reduziu-o para 123/51, H1.5 para 101/51, H1.6 para 88/44 e H1.7 para o teto atual de 66 erros / 27 ficheiros;
...
3. continuar o paydown dos 66 erros TypeScript por domínio; o maior ficheiro isolado é `Financeiro/BancoTab.tsx` com 16 erros, que deve ser tratado num lote próprio por ser fluxo financeiro crítico;
...
|---:|---|---|
| 1 | H1 | H1.1, H1.4, H1.5, H1.6 e H1.7 concluídos/validados por lotes; continuar paydown TypeScript e avançar com Laravel major, migração de `xlsx`, hardening residual R2 e QA frontend. |
...
Próximo passo: continuar H1 por lotes pequenos e mensuráveis, reduzindo a dívida TypeScript a partir do teto 66/27 e mantendo os ratchets de segurança; em paralelo, preparar as mudanças incompatíveis de Laravel e `xlsx` sem degradar o runtime atual.
...
|---|---|---|---|---|
| 2026-08-24 | QA / TypeScript / Inertia | H1.7 normalizou o contrato `PageProps`/`usePage` sem alterar runtime e eliminou todos os 22 `TS2344`, reduzindo a dívida TypeScript de 88 erros/44 ficheiros para 66 erros/27 ficheiros. | PR #214; CI #760; `resources/js/types/index.d.ts`; `qa/baselines/typescript.json`; `docs/qa/H1_BASELINE.md` | Novo teto 66/27. Próximos lotes devem preservar a separação do Financeiro crítico e reduzir novamente o ratchet. |
