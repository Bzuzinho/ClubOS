...
| 2026-06-17 | Financeiro | Sprint F4.5 — exportação e fecho operacional leve da conciliação bancária tecnicamente concluída. Foram adicionados endpoint de exportação detalhada (`GET /financeiro/banco/auditoria/export?format=csv`) e endpoint de resumo operacional (`GET /financeiro/banco/auditoria/export-summary`), ambos com os mesmos filtros da Auditoria e exportação de todas as linhas filtradas (ignorando paginação visual) até limite seguro. A subtab Auditoria passou a expor ações de exportação CSV, resumo CSV e indicador de conferência de período com base em `total_parcial` e `total_por_conciliar`, sem bloqueio transacional e sem mexer no motor financeiro canónico. XLSX permanece desativado por ausência de biblioteca segura já instalada. | `app/Http/Controllers/Financeiro/BankReconciliationAuditController.php`, `app/Services/Financeiro/BankReconciliationAuditService.php`, `routes/web.php`, `resources/js/Components/Configuracoes/Financeiro/BankReconciliationManagementTab.tsx`, `tests/Feature/Financeiro/BankReconciliationAuditEndpointTest.php`, `docs/ESTADO_VIVO_DESENVOLVIMENTO.md`, `docs/PLANO_FECHO_MODULO_FINANCEIRO.md` | 98% | 99% | Tecnicamente concluída; validação manual em browser pendente. Não avançar para F4.6 nesta sprint. |
