...
| Financeiro geral | Faturas, mensalidades, movimentos, extratos, banco, conciliação, relatórios, pedidos fiscais | Membros, Banco, Loja, Logística, Fiscal, Centros de custo | 76% | Sprint F1/F1.1/F1.2 validaram manualmente a liquidação canónica, métodos de pagamento configuráveis, transferência com banco e dinheiro sem banco. |
| Faturas / mensalidades | Geração de mensalidades, faturas abertas, estados, itens, tipos de fatura | Membros, Dados financeiros, centros de custo, pagamentos | 74% | Fluxos de pagamento manual e por transferência foram validados no browser; falta avançar para bugs críticos da F2. |
...
| Conciliação bancária assistida | Sugestões, confirmação, rejeição, alocação de extrato, aliases bancários | Banco, faturas, pagamentos, mapa de conciliação | 72% | Validado que a desconciliação de banco reabre mensalidade e remove pedido fiscal pendente; falta maturar cenários de F4. |
