...
### H5d — Devolução pós-entrega com reversão financeira e fiscal — implementado
...
|---|---|---|---|---|
| 2026-09-03 | Loja / Financeiro / Fiscal / Inventário | H5d cria devolução integral pós-entrega em duas fases quando existe documento fiscal: nota de crédito Wintouch primeiro, depois reversão histórica e reposição idempotente de stock. | `StoreOrderReturnService`; `PaymentReversal`; `StoreOrderDeliveredReturnReversalTest`; audit `h5d-store-return-reversal-audit-v5` | Implementado localmente; aguarda validação Laravel/PostgreSQL/browser na CI, integração e fotografia produtiva. H5e segue após promoção integral. |
