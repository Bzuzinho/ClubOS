...
| Inventário / Logística | 78% | `stock_movements` é o ledger canónico e variantes são atómicas. H5d só repõe stock de uma encomenda entregue depois de concluir as reversões financeira e fiscal, com repetição idempotente. Falta fechar compras/requisições/empréstimos transversalmente em H5e. |
...
Próximo passo após promoção produtiva de H5d: H5e — fechar o lifecycle interno de compras, requisições, entregas e empréstimos de Logística, com QA operacional e contratos Desportivo↔core preservados. H3, H4, H2.5, stock por variante e Família/EE estão estruturalmente fechados. A fila fiscal produtiva e a ação operacional Cloudflare R2 permanecem pendências operacionais separadas.
