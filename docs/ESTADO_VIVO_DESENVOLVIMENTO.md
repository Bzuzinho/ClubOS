...
### H2.5n — Marketing administrativo modular — em validação
...
A declaração extraída é textualmente equivalente após normalização da indentação e `routes/web.php` desceu para `187` linhas, `54` declarações diretas e `12` imports de controllers sem alterar comportamento runtime. Aguarda a validação integral da CI, merge, deploy e artifact produtivo antes de fechar o lote.
...
|---|---|---|---|---|
| 2026-08-31 | Rotas / Marketing | H2.5n extrai a declaração source do resource CRUD administrativo de Marketing para um décimo terceiro módulo controlado, preservando o gate de módulo e adicionando gate explícito para 31 módulos carregados. | `routes/web_marketing.php`; `WebRouteTopologyAuditTest`; `WebRouteTopologyWorkflowContractTest`; workflow CI | Em validação; declaração textualmente equivalente, `routes/web.php` com 187 linhas, 54 declarações diretas e 12 imports. Contrato esperado: hash preservado, 517 rotas, 491 nomes, 31/31 módulos, 23 redirects, 0 referências aos aliases retirados e 0 candidatos literais. H2.5o prepara os resources desportivos adicionais. |
