...
| Inventário / Logística | 84% | `stock_movements` é o ledger canónico e variantes são atómicas. H5e fecha no código o catálogo operacional, elegibilidade backend, retries idempotentes e bloqueios transacionais de compras/requisições/entregas/empréstimos; o audit agregado produtivo e a promoção final ainda estão pendentes. |
| Loja | 84% | H5a–H5c fecharam checkout, cancelamento pré-entrega, fatura, pagamento e fiscalidade. H5d introduz o lifecycle explícito de devolução integral pós-entrega, com nota de crédito quando aplicável, reversões históricas e estado terminal `devolvido`. Permanece o lifecycle interno de Logística em H5e. |
...
### H5e — Lifecycle interno de Logística — implementado, promoção produtiva pendente
...
O novo comando read-only `inventory:audit-internal-logistics-lifecycle` agrega os audits de compras, requisições e empréstimos, mede referências financeiras e pedidos Desportivo→Logística e será recolhido após deploy num artifact sem IDs. `InternalLogisticsLifecycleTest` cobre retry idempotente, preservação da origem Desportivo, momento exato do stock, bloqueio de artigos inelegíveis e contrato do audit. Validação frontend local aprovada; PHP/migrations aguardam a CI por ausência de runtime PHP/Composer local.
...
|---:|---|---|
| 1 | H5 | Loja + Logística lifecycle completo. |
| 2 | H6 | Comunicação assíncrona e futura integração Redes. |
| 3 | H7 | Portal/PWA/mobile. |
| 4 | H8 | Reporting consolidado transversal. |
| 5 | H9 | Website: header/footer, notícias e polish final. |

Próximo passo: promover H5e com CI totalmente verde, validar deploy e recolher o artifact agregado de compras/requisições/entregas/empréstimos; só depois fechar o lote e avançar para H6. H3, H4, H2.5, stock por variante e Família/EE estão estruturalmente fechados. A fila fiscal produtiva e a ação operacional Cloudflare R2 permanecem pendências operacionais separadas.
...
|---|---|---|---|---|
| 2026-09-03 | Logística / Inventário / Financeiro / Desportivo | H5e fecha no código capacidades do catálogo, elegibilidade backend, locks e retries idempotentes em requisições/entregas/empréstimos/compras, preserva a identidade estruturada Desportivo→Logística e acrescenta audit agregado produtivo. | `CanonicalProductStockService`; actions de `Logistica`; `InternalLogisticsLifecycleAuditService`; `InternalLogisticsLifecycleTest`; `Configuracoes/Index.tsx`; `Logistica/Index.tsx`; workflow CI | Implementado; TypeScript 0/0, lint, unit tests e build aprovados localmente. PHP/migrations, PR, deploy e evidência produtiva pendentes. |
