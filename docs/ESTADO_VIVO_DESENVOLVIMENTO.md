...
| Eventos | 76% | Lifecycle, recorrência, convocatórias e integrações corrigidos. O formulário de criação/edição passa a apresentar validação persistente em português, foco no primeiro campo inválido, grelha mobile responsiva e notificações globais efetivamente montadas; um teste de request replica o payload completo enviado pelo browser. H1.18 garante entrada pelo menu em desktop/mobile; faltam CI/Laravel para este bugfix, remoção de estruturas antigas, contract tests Eventos ↔ Desportivo e E2E das restantes operações críticas. |
...
|---|---|---|---|---|
| 2026-09-09 | Eventos / UX / Validação | Corrige o silêncio no popup de criação/edição: monta o sistema global de notificações, mantém erros legíveis dentro do modal, valida datas/recorrência antes do POST, foca o primeiro campo inválido e reorganiza as grelhas para mobile. | `EventosList.tsx`; `app.tsx`; mensagens PT nos Form Requests; testes frontend e payload browser backend | Implementado localmente; lint, 5 testes Vitest, TypeScript e build verdes. PHP não existe no ambiente local; pendente de autorização para push/PR e validação Laravel/CI antes de deploy. |
