...
| Comunicação | 80% | H6a/H6b estão em produção e H6c separa adapters de email/SMS/push, exige ID externo nos providers HTTP e recebe estados `delivered/failed/read` por webhook HMAC idempotente, minimizado e anti-replay. Produção mantém a queue persistente `database`; a ativação de cada callback depende de secrets reais e segue-se a integração Redes. |
...
|---:|---|---|
| 1 | H6 | Comunicação assíncrona e futura integração Redes. |
| 2 | H7 | Portal/PWA/mobile. |
| 3 | H8 | Reporting consolidado transversal. |
| 4 | H9 | Website: header/footer, notícias e polish final. |

Próximo passo: avançar para H6d, integrando Redes como provider adicional do mesmo pipeline sem misturar o Website nem criar uma campanha paralela. A ativação externa dos callbacks H6c exige secrets configurados no ClubOS e no fornecedor. Em paralelo, a campanha legacy agendada/vencida deve ser revista explicitamente; o sistema não a enviará sozinho. Um eventual cutover da queue produtiva `database` para Redis só deve ocorrer depois de validar a configuração efetiva do Supervisor. H3, H4, H5, H2.5, stock por variante e Família/EE estão estruturalmente fechados. A fila fiscal produtiva e a ação operacional Cloudflare R2 permanecem pendências operacionais separadas.
