...
| 2026-06-17 | Financeiro | Sprint F4.4 — relatórios e auditoria operacional da conciliação bancária tecnicamente concluída. Foi criado endpoint de leitura paginado (`GET /financeiro/banco/auditoria`) com filtros por estado/data/pesquisa/método/crédito/ordenação, resumo agregado, detalhe de alocações (faturas, movimentos, crédito), utilizador/data/método de conciliação, histórico de desconciliações e flags operacionais. A UI foi integrada em `Configurações > Financeiro > Conciliação Bancária` na subtab `Auditoria`, com cards de resumo, tabela principal, ação `Ver detalhe` e paginação server-side. Não houve alterações ao motor de pagamentos, alocação, desconciliação, importação XLS ou conta corrente. | `app/Http/Controllers/Financeiro/BankReconciliationAuditController.php`, `app/Services/Financeiro/BankReconciliationAuditService.php`, `routes/web.php`, `resources/js/Components/Configuracoes/Financeiro/BankReconciliationManagementTab.tsx`, `tests/Feature/Financeiro/BankReconciliationAuditEndpointTest.php`, `docs/ESTADO_VIVO_DESENVOLVIMENTO.md`, `docs/PLANO_FECHO_MODULO_FINANCEIRO.md` | 96% | 97% | Estado: tecnicamente concluída, pendente de validação manual no browser. Não avançar para F4.5 nesta sprint. |
