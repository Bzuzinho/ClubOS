...
Na leitura de dívida/conta corrente, `CurrentAccountService` passou a ser a fonte canónica para Dashboard atleta, Portal Profile e `financeiro.invoices.open`. Mantém-se como risco vivo qualquer superfície que ainda apresente dívida fora desta leitura partilhada.
