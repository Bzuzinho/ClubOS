...
| Loja | 72% | H5a fecha o cancelamento e a reposição pré-entrega; H5b substitui o placeholder por uma `Invoice` canónica idempotente, com titular/itens/centro de custo estruturados, pagamento pelo boundary oficial e sem `Movement` ou saída de stock paralelos. Permanecem a projeção automática do pagamento no estado da encomenda e a devolução/reversão posterior à entrega. |
