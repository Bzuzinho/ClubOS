...
| 2026-06-17 | Financeiro | Sprint F4.3 — gestão visual de aliases bancários e rejeições em Configurações > Financeiro > Conciliação Bancária. Foram adicionadas listagens operacionais de aliases (ativos/inativos, associado a utilizador/família, confiança, origem, uso/última utilização) e de sugestões rejeitadas (linha bancária, score, motivo, rejeitado em/por), com ações de desativar/reativar alias e limpar rejeição para permitir nova geração. A desativação de alias foi implementada sem migration nova através de marcação segura no campo `source` e o motor de sugestões passou a ignorar aliases desativados. A limpeza de rejeição mantém trilho mínimo em `metadata.rejection_clears`, sem apagar histórico. Refinamento posterior na mesma sprint: paginação server-side aplicada nas duas grelhas de gestão (aliases e rejeições) para reduzir carga e melhorar leitura operacional em volumes altos. | `app/Http/Controllers/Financeiro/BankReconciliationAliasController.php`, `app/Http/Controllers/Financeiro/BankReconciliationSuggestionController.php`, `app/Services/Financeiro/ReconciliationAliasService.php`, `routes/web.php`, `resources/js/Components/Configuracoes/Financeiro/BankReconciliationManagementTab.tsx`, `resources/js/Pages/Configuracoes/Index.tsx`, `tests/Feature/Financeiro/BankReconciliationAliasManagementTest.php`, `docs/PLANO_FECHO_MODULO_FINANCEIRO.md` | 94% | 95% | Tecnicamente concluída; pendente de validação manual orientada no browser. Não avançar para F4.4 nesta sprint. |
