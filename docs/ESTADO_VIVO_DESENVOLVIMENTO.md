...
| Financeiro geral | Faturas, mensalidades, movimentos, extratos, banco, conciliação, relatórios, pedidos fiscais | Membros, Banco, Loja, Logística, Fiscal, Centros de custo | 84% | F2.4 corrigiu de forma limitada o popup `Liquidar Movimento`: deixou de obrigar `numero_recibo`, passou a usar apenas métodos ativos e a mostrar linhas bancárias apenas para métodos com `requer_linha_bancaria`, sem alterar o fluxo canónico de Faturas/Mensalidades. F2/F2.1/F2.2/F2.3/F2.4 continuam pendentes de validação manual dirigida no browser até repetição dos testes. |
...
| Testes e qualidade | Testes financeiros identificados, nomeadamente alocação de pagamentos | Financeiro, regressões | 39% | A cobertura Financeiro inclui regressões explícitas para pagamento canónico de faturas `material`, `inscricao` e `mensalidade` sem nº de recibo, garante que a Emissão Fiscal continua a exigir `external_document_number` no `mark-issued`, tem contract test da tab Faturas e passou a cobrir `liquidarMovimento` sem nº de recibo, com método inativo rejeitado e transferência dependente de linha bancária. Continua insuficiente face ao tamanho da aplicação. |
