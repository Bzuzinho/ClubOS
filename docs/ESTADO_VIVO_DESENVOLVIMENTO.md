...
| Eventos | 80% | Lifecycle, recorrência, convocatórias e integrações corrigidos. O evento passa a guardar explicitamente o público canónico (`todos`, atletas, EE e outros utilizadores), combinável sem confundir escalão com destinatário; os escalões filtram apenas atletas. Portal e segmentos de Comunicação usam o mesmo `EventAudienceResolver`, incluindo atletas/desportivo e EE/família a partir das fontes canónicas. O formulário mantém validação persistente, grelha mobile e notificações globais. Permanecem a remoção de estruturas antigas, contract tests Eventos ↔ Desportivo e E2E das restantes operações críticas. |
| Financeiro geral | 91% | Maduro; CRUDs legacy aposentados e liquidações convergem no fluxo canónico. H5d acrescenta `payment_reversals` imutáveis e cancela logicamente alocações, lançamentos e mapas sem apagar o rasto original; a compensação financeira da devolução fica explícita e idempotente. A tab Mensalidades expõe a linha bancária que suportou cada conciliação ativa, incluindo data, descrição, referência/conta, valor imputado e valor total do movimento quando existe repartição, sem apresentar mapas anulados/revertidos como conciliações válidas. A edição administrativa de mensalidades preserva agora a origem canónica/histórica (`monthly_fee`/`monthly_fee_legacy`) sem a reenviar como campo editável, eliminando o 422 em mensalidades legacy. |
...
### H3f — Portal operacional — concluído
...
### H4 — Decisão e fecho Fiscal — concluído
...
O novo `finance:audit-fiscal-operational-readiness` é read-only e agregado: confirma configuração, schema, sete rotas operacionais, ausência de adapters automáticos e zero findings críticos do audit fiscal. A CI recolhe o artifact após deploy e exige `summary.ready=true`.
...
### H5b — Fatura canónica da Loja — concluído em produção

A submissão de uma encomenda com valor positivo passa a criar, na mesma transação, uma única `Invoice` `material` com origem `store_order`, saldo em aberto, vencimento a 15 dias e itens derivados dos snapshots da encomenda. O titular é o perfil beneficiário (`target_user_id`) quando aplicável; o centro de custo segue o resolver canónico e só é escolhido quando existe uma decisão determinística. A fatura nasce oculta, recebe os itens e só depois é publicada.

O lock da encomenda e o índice parcial único `(origem_tipo, origem_id)` para `store_order` tornam `prepareForOrder()` idempotente. A saída física permanece exclusivamente em `store_order_item`; entregar uma encomenda canónica deixa de criar o `Movement` de receita legado. A liquidação converge pelo `PaymentAllocationService`, produz uma única receita e mantém o fiscal no contrato `manual_wintouch`. O cancelamento pré-entrega continua possível apenas quando a fatura está virgem: a obrigação é cancelada logicamente e o stock é reposto na mesma transação; qualquer rasto financeiro/fiscal bloqueia até à reversão H5d.

O audit passa para `h5b-store-logistics-financial-audit-v3`, acrescentando coerência de ligação, titular, valor e itens. Não foi feito backfill da encomenda histórica. PR #301 merged em `854624bc2f91829555f70c6dbff0a58a6f2c8067`; CI #1084 ficou totalmente verde na PR e CI #1085 repetiu Laravel, PostgreSQL concorrente e browser QA, aplicou a migration, fez deploy na Oracle VM e executou as auditorias. O artifact `store-logistics-lifecycle-readiness-854624bc2f91829555f70c6dbff0a58a6f2c8067` (ID `9858357355`, `sha256:b94629d7dc35e59c5c292f95409aabbe80c2ff76d890977c15deaefcbb94395c`) confirmou uma encomenda/um item legacy, zero referências de fatura inválidas, zero contratos/items divergentes, zero saídas ausentes/duplicadas, zero críticos, zero warnings, zero ações e `no_data_changed=true`.

### H5c — Projeção de pagamento e fiscalidade da Loja — concluído em produção

`StoreOrderFinancialProjection` mantém quatro dimensões explícitas e independentes: estado logístico, estado de pagamento, saldo e estado fiscal. Pagamento/saldo são sempre derivados da `Invoice`; fiscalidade é derivada do pedido associado. Não foi criada qualquer coluna financeira em `loja_encomendas`, nem a liquidação muda implicitamente `pendente/aprovado/preparado/entregue`.

As APIs e interfaces do utilizador e administração apresentam pagamento pendente/parcial/pago, valores pago/em aberto e o percurso `aguarda_pagamento → pendente_emissao/em_emissao → emitido` no contrato `manual_wintouch`. O teste ponta a ponta liquida a fatura em duas alocações pelo `FinancialSettlementService`/`PaymentAllocationService`, confirma criação idempotente do pedido `receipt` para `wintouch`, regista o número externo e prova que a encomenda conserva o estado logístico, uma única saída física e zero `Movement` paralelo.

O audit produtivo passa para `h5c-store-payment-fiscal-audit-v4`: compara `Invoice` com alocações confirmadas e mede pedidos Wintouch criados, ausentes, prematuros ou com contrato divergente. Continua read-only, agregado, sem backfill e sem IDs no artifact.

PR #303 foi integrada no merge `e7ddd8884d095bda5adf521444f0af2bbb487031`; CI #1088 ficou totalmente verde na PR e CI #1089 repetiu Laravel, PostgreSQL concorrente e browser QA, fez deploy na Oracle VM e recolheu as auditorias. O artifact `store-logistics-lifecycle-readiness-e7ddd8884d095bda5adf521444f0af2bbb487031` (ID `9865860191`, `sha256:db2bb47cd045a85716fdbcabab389f6f979a6de30176c74c789810d2606b8649`) confirmou uma encomenda/um item legacy, zero projeções incoerentes, zero pedidos fiscais ausentes/prematuros/divergentes, zero saídas ausentes/duplicadas, zero críticos, zero warnings, zero ações e `no_data_changed=true`. Os contadores canónicos de pagamento/fiscal ficam a zero porque a única encomenda produtiva é anterior a H5b e não foi criado dado sintético para validar o lote.

### H5d — Devolução pós-entrega com reversão financeira e fiscal — concluído em produção

A encomenda entregue passa a ter um fluxo próprio de devolução, separado da simples mudança de estado. `loja_encomenda_devolucoes` conserva motivo, operador e marcos de reversão; `devolvido` é terminal e só é gravado depois de terminar toda a sequência.

Quando existe recibo externo, a primeira ação cria idempotentemente um pedido `credit_note` para o mesmo provider Wintouch e mantém pagamento e stock intactos. A conclusão só é permitida depois de o operador emitir a nota de crédito no Wintouch e registar o respetivo número no ClubOS; o recibo original é então cancelado logicamente, mas ambas as identidades externas permanecem no histórico.

`PaymentAllocationService::reverseInvoicePaymentsPreservingHistory()` cria um `PaymentReversal` por alocação, marca alocações, lançamentos e mapas como cancelados/revertidos sem os apagar, impede que o valor revertido reapareça como saldo disponível e cria a compensação de reembolso. Apenas depois desta reversão e do fecho fiscal o ledger regista a reposição de produto/variante e a encomenda transita para `devolvido`. Repetir a operação não duplica nota de crédito, reversão, lançamento ou stock.

O audit produtivo evolui para `h5d-store-return-reversal-audit-v5`, exige o novo schema e mede devoluções em espera, concluídas/incoerentes, reversões preservadas e stock devolvido desequilibrado. O teste bloqueante `StoreOrderDeliveredReturnReversalTest` cobre checkout → entrega → pagamento → recibo → pedido/registo de nota de crédito → reversão → reposição, incluindo o bloqueio intermédio e idempotência.

PR #305 foi integrada no merge `b178ec5426f78a845995e5173d3a60179d06b7d9`; CI #1092 validou a PR e CI #1095 repetiu todos os gates no HEAD produtivo `6a0aa5d1ec8003613138931ad790654f7ac0f037`, aplicou a migration, fez cutover atómico na Oracle VM e recolheu as auditorias. O artifact `store-logistics-lifecycle-readiness-6a0aa5d1ec8003613138931ad790654f7ac0f037` (ID `9884759118`, `sha256:5c085d60091f92a8d01a6f8746aab4ed653af797677330af932c1efa6a9dc82d`) confirmou o schema H5d, uma encomenda/um item legacy, zero devoluções inconsistentes, zero stock devolvido desequilibrado, zero críticos, zero warnings, zero ações e `no_data_changed=true`.

### H5e — Lifecycle interno de Logística — concluído em produção

`Configurações > Logística > Artigos` passa a gerir explicitamente se cada artigo pode ser requisitado, emprestado e se tem stock controlado. As seleções da UI e os services aplicam o mesmo contrato, impedindo bypass por request direto. Compras a fornecedor aceitam apenas artigos ativos com gestão de stock. Edições no Admin da Loja preservam as capacidades logísticas e a remoção em Configurações desativa o artigo sem apagar o ledger histórico.

Aprovação e entrega de requisições e devolução de empréstimos passam a bloquear o agregado e a usar chaves idempotentes no `StockLedgerService`; retries já concluídos não duplicam reservas, saídas ou retornos. Updates/deletes de empréstimos e compras bloqueiam o registo antes de calcular deltas. O gateway Desportivo mantém identidade estruturada e criação sem stock; aprovação reserva e entrega consome fisicamente.

O novo comando read-only `inventory:audit-internal-logistics-lifecycle` agrega os audits de compras, requisições e empréstimos, mede referências financeiras e pedidos Desportivo→Logística e é recolhido após deploy num artifact sem IDs. `InternalLogisticsLifecycleTest` cobre retry idempotente, preservação da origem Desportivo, momento exato do stock, bloqueio de artigos inelegíveis e contrato do audit.
...
`communication:dispatch-due` corre a cada minuto e enfileira campanhas H6a agendadas, retries vencidos e leases abandonadas. Campanhas históricas sem chave nunca são disparadas automaticamente. Cada destinatário tem no máximo três tentativas, separadas por backoff de 1 e 5 minutos; exceções do job usam 1/5/15 minutos. Email usa `Message-ID` determinístico, SMS propaga `Idempotency-Key`, alertas internos guardam o ID do registo e push falha fechado sem provider. A configuração Laravel passa a expor a ligação Redis e os jobs só são publicados após commit.

A vista Execução mostra tentativas, retries e esgotamentos. O audit read-only `communication:audit-async-pipeline` mede schema, legado, backlog, referências de provider e anomalias; a CI recolhe apenas schema e contagens agregadas após deploy. Não existe backfill ou mutação de histórico. O contrato e os limites de exactly-once ficam em `docs/modules/communication_async_pipeline.md`.

PR #309 foi integrada no merge `da108ba6d4df33a216c5584ce28f8a3fe939ce67`. CI #1104 validou a PR e CI #1105 repetiu Laravel, PostgreSQL concorrente e browser QA em `main`, aplicou a migration, reiniciou a queue e concluiu o deploy atómico na Oracle VM. O artifact `communication-async-pipeline-readiness-da108ba6d4df33a216c5584ce28f8a3fe939ce67` (ID `9892129492`, `sha256:23c4dc640b8011ec734f1ef925985ad5f25d094125de02a01c0e678bda2e0270`) confirmou schema completo e zero críticos, retries vencidos/em espera, esgotamentos ou leases abandonadas. Mediu 65 campanhas, 123 entregas e 7991 destinatários legacy, zero registos H6a ainda executados e uma campanha legacy agendada/vencida, protegida contra envio automático e pendente de revisão explícita. A queue produtiva observada é `database`; a ligação Redis está disponível, mas o seu eventual cutover exige validação operacional prévia do Supervisor.

### H6b — Cutover assíncrono das automações — integrado e deployado

`CommunicationAutomationService` deixa de executar entregas inline para faturas, movimentos, convocatórias de Eventos e fluxos de Logística. `SportsCommunicationGatewayService` aplica o mesmo contrato à publicação Desportivo. Ambos persistem a campanha idempotente e chamam `queueIndividualCommunication`, que fixa canais conforme as preferências existentes e publica o job apenas depois do commit da transação funcional.

Se o evento de negócio fizer rollback, não subsiste campanha nem job. Se uma falha ocorrer depois de persistir uma campanha automática e antes de existir uma entrega, `communication:dispatch-due` recupera outbox em `rascunho` sem dispatch ou `em_processamento` há mais de dez minutos. O audit evolui para `h6b-communication-automation-cutover-audit-v2` e mede campanhas automáticas, processamento, ausência de dispatch e backlog recuperável. O envio individual explícito e os lifecycles próprios de mensagens internas/alertas operacionais fora do serviço de automações não são alterados neste lote.

PR #311 foi integrada no merge `407ee6f825bbdadba27b6d02f95d2bba18a802c8`. CI #1109 validou a PR e CI #1110 repetiu Laravel, PostgreSQL concorrente e browser QA no commit de `main`, concluiu o deploy atómico na Oracle VM e recolheu o artifact `communication-async-pipeline-readiness-407ee6f825bbdadba27b6d02f95d2bba18a802c8` (ID `9897116395`, `sha256:e947ad5bac55e2f8544347b21d4952189e2061312d6a019aca3ccbb49a3f9cf2`). O audit produtivo confirmou schema completo, queue ativa `database`, zero campanhas automáticas ainda criadas, zero automações sem pedido de dispatch, zero outbox por recuperar, zero retries, esgotamentos ou leases abandonadas e zero críticos. Conservam-se 65 campanhas, 123 entregas e 7991 destinatários legacy sem backfill; a única agenda legacy continua protegida contra envio automático e permanece a única ação operacional.

### H6c — Adapters e webhooks autenticados dos providers — integrado e deployado

Email, SMS e push passam por adapters explícitos com resultado normalizado; o serviço de entregas deixa de conhecer APIs HTTP ou detalhes do mailer. SMS e push propagam idempotência e só aceitam sucesso quando existe `provider_message_id`. Push continua fail-closed sem endpoint/token e a resolução de destinatários mantém-se bloqueada enquanto não existir registo canónico de tokens.

O novo endpoint `POST /api/webhooks/communication/{provider}` aceita apenas `email`, `sms` e `push`, com throttling, secret por provider, HMAC-SHA256 e janela anti-replay. `communication_provider_events` guarda apenas identidade externa, estado, timestamps, hash do payload e correlação; não persiste o payload bruto. Duplicados são no-op, identidades reutilizadas com conteúdo divergente são recusadas e eventos sem correlação ficam medidos. Estados avançam de forma monotónica e `failed` não rebaixa `delivered/read`; entrega e campanha são recalculadas após cada transição aplicada.

O audit evolui para `h6c-communication-provider-lifecycle-audit-v3`, exige o schema de eventos e mede eventos aplicados, ignorados e sem correlação. Secrets e endpoints reais não são inventados pelo deploy: cada provider permanece inativo/fail-closed até configuração operacional explícita.

PR #313 foi integrada no merge `6cd2103e0897ac8f7909676e9e728e134826e709`. CI #1113 validou a PR e CI #1114 repetiu Laravel, PostgreSQL concorrente e browser QA no commit de `main`, aplicou a migration, concluiu o deploy atómico na Oracle VM e recolheu o artifact `communication-async-pipeline-readiness-6cd2103e0897ac8f7909676e9e728e134826e709` (ID `9900110232`, `sha256:84ff2204e3459f6b6da72080bfe4c38022e090b4aaabf36a1ed8300db4b04283`). O audit produtivo confirmou as cinco tabelas e todos os campos H6c, zero eventos de provider/aplicados/ignorados/sem correlação, zero críticos, zero retries/esgotamentos/leases e ausência de mutação. Os secrets de email, SMS e push estão deliberadamente por configurar, mantendo os callbacks fail-closed; a única ação continua a campanha legacy agendada protegida contra envio automático.

### H6d — Facebook e Instagram no pipeline canónico — integrado e deployado

`Definições > Notificações > Redes sociais` passa a gerir uma conta Facebook e uma conta profissional Instagram. IDs, versão Graph API e estado ficam operacionais; access token, app secret e verify token são cifrados e nunca regressam ao browser. Guardar valores secretos em branco preserva os existentes, desligar remove as credenciais e a validação consulta a API oficial antes de marcar a ligação como verificada.

`Comunicação > Redes` cria publicações imediatas ou agendadas em `communication_campaigns`; não existe tabela paralela de campanhas nem acoplamento ao Website. Facebook publica texto/ligação em `/feed`; Instagram cria e publica um contentor com imagem pública. Os dois canais usam o job, deliveries, destinatários, attempts, backoff e IDs externos já existentes. `GET|POST /api/webhooks/meta/{provider}` suporta challenge, assinatura `X-Hub-Signature-256`, idempotência e eventos minimizados sem payload bruto.

O audit evolui para `h6d-social-network-publishing-audit-v4` e exige contas, eventos, ligação do destinatário social e interpretações de segurança/fronteira. A ativação real continua deliberadamente pendente das credenciais Meta e da configuração dos callbacks na aplicação Meta; sem elas os adapters e webhooks falham fechados.

PR #315 foi validada integralmente pela CI #1117 e integrada no merge `cbc879eb320a42a691b3fd1c1a14be50b2810c00`. A CI #1118 repetiu PostgreSQL concorrente, Laravel, lint, testes frontend, TypeScript, build e browser QA multi-browser/mobile, concluiu o deploy atómico na Oracle VM e recolheu o artifact `communication-async-pipeline-readiness-cbc879eb320a42a691b3fd1c1a14be50b2810c00` (ID `9903209342`, `sha256:775cc80bdf23f167fda7063f573e53b155597101cd759d1a570548008039b649`).

O audit produtivo confirmou as sete tabelas e todos os campos H6d, `schema_ready=true`, queue ativa `database`, zero contas sociais, zero contas prontas para publicar, zero campanhas/eventos sociais e zero críticos. As credenciais reais não foram transportadas nem inventadas pelo deploy, pelo que este estado é o esperado antes da configuração operacional. Mantêm-se 65 campanhas, 123 entregas e 7991 destinatários legacy sem backfill; a única campanha legacy agendada continua a única ação/warning e permanece bloqueada contra envio automático. O audit foi read-only e confirmou `no_data_changed=true`.
...
|---|---|---|---|---|
| 2026-09-14 | P1 / Importação de membros | CSV preserva acentos UTF-8 com/sem BOM e Windows-1252; diálogo protege leitura, validação e importação em curso contra fecho, alteração de mapeamento, navegação e submissões concorrentes; apresenta progresso e desbloqueia após sucesso/erro. | `MemberImportDialog`; testes de componente com pedidos pendentes; E2E de pré-visualização CSV preenchida | Validação local/CI em curso. Sem alterações backend, migrations ou dados produtivos. Não garante idempotência entre separadores/reloads; importação efetiva em navegador permanece pendente. |
| 2026-09-14 | Consolidação P1/P3 | PRs #349–#353 integradas em `44c2655`, incluindo ficha, histórico, Cais/Live, Análise canónica e resultados oficiais/filtro de épocas. | CI/deploy 34618321605; PR #353 | Produção confirmada em 11/09. Browser do merge: 184 passaram diretamente e recuperação de palavra-passe Firefox passou na repetição. Registos anteriores “CI pendente” descrevem o estado de cada lote antes da integração. P1/P2/P3/P4 não estão globalmente fechadas. |
| 2026-09-11 | P3 / Resultados individuais e épocas | Ficha acrescenta resultados oficiais paginados de `results → provas → competitions`, com estado, tempo, classificação, pontos e parciais, preservando os registos owned por Eventos. Treinos ganham filtro pelas épocas das participações reais. | `CompetitionResultController`; `CompetitionHistory`; `GetAthleteTrainingHistory`; testes funcionais e fixture/browser | CI pendente; incremento sobre #352, com #349–#351 por integrar. Sem migrations ou alteração de permissões. Competições não têm ligação explícita a época: não é inferida pela data. Unificação/migração dos resultados de Eventos, snapshot individual e P1/P2/P4 permanecem pendentes. |
| 2026-09-11 | P3 / Análise ↔ Cais | Análise individual e cobertura de grupo passam a ler `training_metrics`, a mesma fonte gravada pelo Cais; catálogo mantém nomes/unidades e a projeção continua sem writes. | `SportsAnalysisWorkspaceService`; `SportsCaisWorkspaceService`; `SportsAnalysisWorkspaceFunctionalTest`; matriz transversal | CI pendente; incremento sobre #351, com #349/#350 também abertas. Sem migrations, alterações de permissões ou percentagens. Tabela/modelo de transição permanecem fisicamente até verificação separada do cutover produtivo; resultados e consolidação histórica por época pendentes. |
| 2026-09-11 | P3 / Registos individuais Cais e Live | Detalhe por participação na ficha reutiliza Registos; tempos/parciais, métricas Live válidas e notas Cais, com autorização existente. | `SportsRecordsReadModelService`; `TrainingRecords`; testes funcionais de Registos e E2E | CI pendente; incremento sobre #350. Sem migrations, writes ou percentagens. Análise ainda lê métricas Cais de outra tabela; alinhar essa projeção. Resultados e historial individual de escalão pendentes. |
| 2026-09-11 | P3 / Histórico de treinos do atleta | Ficha consulta participações canónicas paginadas, independentemente do escalão atual; época e presença do Cais visíveis; estado real da sessão substitui conclusão inferida pela data. | `GetAthleteTrainingHistory`; `TrainingController`; `TreinosTab`; `AthleteTrainingHistoryTest`; E2E | TypeScript/lint locais verdes; CI pendente. Incremento sobre #349 ainda aberta. Sem migrations, writes produtivos, alterações de permissões ou percentagens. Snapshots individuais, Live/métricas/resultados consolidados e restantes pendências da auditoria continuam abertos. |
| 2026-09-10 | P1 / Ficha desportiva | Corrige pesquisa de Membros para incluir email, além de email_utilizador. Cinco tabelas adaptáveis e navegação comum; campos de treino e todas as provas visíveis; elimina duplicação móvel; E2E de ficha com treino preenchido. | `Members/Tabs/Sports*`; `E2eSportsBrowserSeeder`; `authenticated-access.spec.ts`; matriz P1 | TypeScript/lint locais verdes; CI pendente. PR #348 integrada em `efbcf56`. Sem alterações de dados produtivos ou percentagens. Histórico individual ainda limitado: treinos por escalão atual e presenças apenas de Eventos; integração canónica por participação pendente. |
| 2026-09-10 | P1 / Tabelas residuais | Adapta 22 tabelas em Financeiro, Membros, Configurações, Comunicação e Loja; E2E inclui contentores de tabelas nativas; inventário distingue consumidores ativos, ramos sem referências e HTML de exportação. | Componentes indicados em `docs/qa/P1_MATRIZ_TRANSVERSAL.md`; `authenticated-access.spec.ts` | CI pendente. PR #347 integrada em `6cf1493`. Sem alterações de dados, regras ou percentagens; ficha desportiva de Membros, imports preenchidos e diálogos residuais pendentes. |
| 2026-09-10 | P1 / Editor Website | Editor no layout comum com menu acessível; painéis adaptáveis, preview ajustada ao espaço e ferramentas visíveis em mobile. Fixture de rascunho e E2E dos seis separadores/três dispositivos. | `Website/Pages/Edit.tsx`; `E2eBrowserTestSeeder`; `authenticated-access.spec.ts`; matriz P1 | CI pendente. PR #346 integrada em `3c7f355`. Sem alterações de dados produtivos, publicação, regras de negócio ou percentagens; persistência Website em browser pendente. |
| 2026-09-10 | P1 / Desportivo preenchido | Fixture testing com época/ciclos/treino/atleta/série; browser verifica Planeamento, Cais Lista/Cards e seleção atleta→série no Live. Corrige larguras mínimas de grelhas e ações no Cais/Live preenchido. | `E2eSportsBrowserSeeder`; `authenticated-access.spec.ts`; páginas Planeamento/Cais/Live e componentes Cais | TypeScript e lint locais passaram; CI pendente. PR #345 integrada em `4e3e71a`. Sem alterações de dados produtivos, regras desportivas ou percentagens; Website e persistência operacional em browser pendentes. |
| 2026-09-10 | P1 / Percursos preenchidos | Fixture financeira isolada em testing e percurso browser de conciliação manual com fatura, edição de alocação e cancelamento; overflow verificado em cada separador visitado; dez tabelas de Estrutura Desportiva adaptadas após falha móvel na CI. | `E2eBrowserTestSeeder`; `authenticated-access.spec.ts`; `docs/qa/P1_MATRIZ_TRANSVERSAL.md` | CI pendente. PR #344 integrada em `84d6029`; deploy acompanhado separadamente. Sem alterações de dados produtivos, regras financeiras ou percentagens. Ciclos, Cais/Live e Website permanecem pendentes. |
| 2026-09-10 | P1 / Matriz transversal | Retoma o lote global: matriz por módulo; separadores internos comuns; tabelas Membros/Financeiro/Portal; largura útil do Desportivo; seletores operacionais sem deslocação horizontal; percursos autenticados por módulo na CI. | `docs/qa/P1_MATRIZ_TRANSVERSAL.md`; `authenticated-access.spec.ts`; componentes e páginas referenciados na matriz | TypeScript/lint/build locais; CI do lote pendente. P1 não declarada fechada: faltam percursos com dados, editor Website e revisão de scroll vertical. Sem alterações de dados, rotas, migrations ou percentagens. |
| 2026-09-10 | Loja / Membros / UX-P1 | Catálogo, encomendas, relatório detalhado de membros e tabela de comunicações reutilizam as tabelas adaptáveis, com campos identificados e ações preservadas. Exportações e regras de negócio mantidas. | `AdminProductList`; `AdminOrdersTable`; `ReportsTab`; `CommunicationsTab` | TypeScript, lint, teste do componente Table e build locais verdes; CI pendente. PR #342 integrada em main; acompanhar deployment. Sem alterações de dados, rotas ou percentagens. Restantes tabelas e contexto modalidade/época pendentes. |
| 2026-09-10 | UX-P1 transversal | Navegação comum em Logística, Loja, Patrocínios e Configurações; tabelas adaptáveis em Logística, Patrocínios e Treinos, campos identificados e ações preservadas; filtros sem colunas fixas excessivas. | `ModuleTabsList`; `table.tsx`; `app.css`; `responsive-table-browser.mjs` | TypeScript, lint, testes de componentes e build locais verdes. Gate de browser a 320/375/768/1280/1920 px acrescentado à CI; download local do Chromium indisponível. Restantes tabelas e validação funcional profunda pendentes. Sem alterações de dados, rotas, permissões ou percentagens. |
| 2026-09-10 | Desportivo / Navegação | Navegação comum com sete destinos canónicos e filtragem pelas permissões das rotas; largura total por defeito no layout administrativo, sem botão nem estado de Maximizar/Repor menu, com menu principal acessível e adaptação automática ao telemóvel; retirada do menu duplicado de Treinos e Biblioteca contextual preservada. | `SportsNavigation.php`; `SportsNavigation.tsx`; `AuthenticatedLayout`; PR #341 | PR #341 integrada e publicada; CI e deployment verdes. Contexto global modalidade/época e normalização interna dos restantes ecrãs pendentes; sem migrations ou mudanças de dados. |
| 2026-09-10 | Frontend / Conceito visual | Mockup conceptual aprovado; preservar linguagem visual do ClubOS. PR #341 substitui grelha fixa por tabs flexíveis sem scroll horizontal. | `ModuleTabsList`; contrato visual | CI e QA visual pendentes. Contexto global real e normalização interna por implementar; percentagens inalteradas. |
...
| 2026-09-09 | Logística / Loja / Catálogo / Inventário | H5f fixa duas experiências sobre uma fonte de verdade: ownership explícito de campos, elegibilidade única de venda, stock apenas por ledger, categorias/custos canónicos, destaques únicos, estados de encomenda monotónicos e proteção fail-closed para variantes. Remove implementações legacy sem consumidores. | PR #335; CI #1177/#1178; merge `eb4af3bd56f98a59a552206cdcb1c1e92c02098e`; `docs/architecture/logistica_loja_canonical_boundaries.md` | Integrado e deployado automaticamente; validação, concorrência PostgreSQL, browser QA e job de produção verdes. |
...
| 2026-09-03 | Comunicação / Financeiro / Eventos / Logística / Desportivo | H6b substitui entregas inline das automações e do gateway Desportivo por campanha-outbox + job após commit, preserva preferências/origens/idempotência e recupera campanhas automáticas sem entrega. | PR #311; CI #1109/#1110; merge `407ee6f825bbdadba27b6d02f95d2bba18a802c8`; artifact `communication-async-pipeline-readiness-407ee6f825bbdadba27b6d02f95d2bba18a802c8` | Integrado e deployado; audit v2 com schema completo, 0 automações sem dispatch, 0 outbox recuperável, 0 retries/esgotamentos/leases e 0 críticos. Mantém 65 campanhas/123 entregas/7991 destinatários legacy e 1 agenda legacy protegida. Comunicação 70% → 75%; H6c é o próximo lote. |
| 2026-09-03 | Comunicação / Infraestrutura | H6a cria dispatcher de agendamentos/retries, attempts persistentes, leases, idempotência campanha→canal→destinatário, provider IDs, proteção de agendas legacy, observabilidade UI e audit produtivo agregado. | PR #309; CI #1104/#1105; merge `da108ba6d4df33a216c5584ce28f8a3fe939ce67`; artifact `communication-async-pipeline-readiness-da108ba6d4df33a216c5584ce28f8a3fe939ce67` | Integrado e deployado; schema completo, zero críticos/retries/esgotamentos/leases, 65 campanhas/123 entregas/7991 destinatários legacy e 1 agenda legacy protegida para revisão. Queue ativa `database`; Redis configurado mas sem cutover implícito. H6b é o próximo lote. |
...
| 2026-09-02 | Loja / Financeiro / Inventário | H5b cria uma `Invoice` canónica e idempotente por encomenda, com beneficiário, itens e centro de custo estruturados; remove o `Movement` paralelo das novas entregas e mantém a saída física apenas no checkout. | PR #301; CI #1084/#1085; merge `854624bc2f91829555f70c6dbff0a58a6f2c8067`; artifact `store-logistics-lifecycle-readiness-854624bc2f91829555f70c6dbff0a58a6f2c8067` | Integrado e deployado com índice parcial de idempotência; 1 encomenda/1 item legacy, 0 ligações inválidas, 0 divergências, 0 saídas ausentes/duplicadas e 0 críticos/warnings/ações. H5c avança para projeção de pagamento + fiscal. |
