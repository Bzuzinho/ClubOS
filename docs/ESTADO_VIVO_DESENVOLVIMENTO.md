...
### H2.5p — Financeiro administrativo complementar modular — em validação
...
O bloco extraído é textualmente equivalente após normalização da indentação e `routes/web.php` desceu para `117` linhas, `14` declarações diretas e `6` imports de controllers sem alterar comportamento runtime. Aguarda a validação integral da CI, merge, deploy e artifact produtivo antes de fechar o lote.
...
|---|---|---|---|---|
| 2026-08-31 | Rotas / Financeiro | H2.5p extrai as 36 declarações administrativas complementares de transações, categorias, movimentos, extratos e pedidos fiscais para um décimo quinto módulo controlado, preservando gates e permissões e adicionando gate explícito para 33 módulos carregados. | `routes/web_finance_complementary.php`; `WebRouteTopologyAuditTest`; `WebRouteTopologyWorkflowContractTest`; workflow CI | Em validação; bloco textualmente equivalente, `routes/web.php` com 117 linhas, 14 declarações diretas e 6 imports. Contrato esperado: hash preservado, 517 rotas, 491 nomes, 33/33 módulos, 23 redirects, 0 referências aos aliases retirados e 0 candidatos literais. H2.5q prepara a fronteira pública/PWA inicial. |
