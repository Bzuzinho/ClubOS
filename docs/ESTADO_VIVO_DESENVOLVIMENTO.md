...
| Financeiro geral | Faturas, mensalidades, movimentos, extratos, banco, conciliação, relatórios, pedidos fiscais | Membros, Banco, Loja, Logística, Fiscal, Centros de custo | 80% | Sprint F2.1 removeu a dependência do nº de recibo/Wintouch para liquidar, passou a criar pedido fiscal pendente no pagamento total e introduziu reabertura canónica segura antes da emissão fiscal. F2/F2.1 continuam pendentes de validação manual dirigida no browser. |
| Faturas / mensalidades | Geração de mensalidades, faturas abertas, estados, itens, tipos de fatura | Membros, Dados financeiros, centros de custo, pagamentos | 80% | Faturas manuais respeitam o tipo escolhido, já podem ser reabertas pelo endpoint canónico antes de existir documento fiscal externo e o update direto de reabertura continua bloqueado. |
| Pagamentos e alocação canónica | `PaymentAllocationService`, criação de pagamentos, alocação a faturas, recalculo de estados, créditos, pedido fiscal automático | Faturas, banco, mapa de conciliação, pedidos fiscais | 86% | Transferência sem linha bancária bloqueia; transferência com linha bancária liquida/concilia; dinheiro sem linha bancária liquida pelo fluxo canónico; pagamento total de fatura manual cria pedido fiscal pendente sem exigir nº de recibo. |
