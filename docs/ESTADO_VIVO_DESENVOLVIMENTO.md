...
| Comunicação | 75% | H6a está em produção e H6b faz o cutover de Financeiro, Eventos, Logística e Desportivo para a outbox: o processo funcional fixa preferências/canais e o job só é publicado após commit, com recuperação de campanhas automáticas órfãs. Produção mantém a queue persistente `database`; seguem adapters/webhooks reais e futura integração Redes. |
...
Próximo passo: avançar para H6c (adapters/providers e webhooks autenticados) sobre a outbox H6b já produtiva. Em paralelo, a campanha legacy agendada/vencida deve ser revista explicitamente; o sistema não a enviará sozinho. Um eventual cutover da queue produtiva `database` para Redis só deve ocorrer depois de validar a configuração efetiva do Supervisor. H3, H4, H5, H2.5, stock por variante e Família/EE estão estruturalmente fechados. A fila fiscal produtiva e a ação operacional Cloudflare R2 permanecem pendências operacionais separadas.
