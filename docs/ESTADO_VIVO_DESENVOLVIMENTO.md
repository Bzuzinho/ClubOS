...
| Financeiro geral | Faturas, mensalidades, movimentos, extratos, banco, conciliação, relatórios, pedidos fiscais | Membros, Banco, Loja, Logística, Fiscal, Centros de custo | 83% | A correção final de F2.3 fechou o modal canónico de pagamento em Faturas: a tab recebe o estado completo de faturas, filtra apenas métodos ativos, mostra linhas bancárias só quando o método ativo o exige e mantém `numero_recibo` reservado à Emissão Fiscal. F2/F2.1/F2.2/F2.3 continuam pendentes de validação manual dirigida no browser até repetição dos testes. |
...
| Testes e qualidade | Testes financeiros identificados, nomeadamente alocação de pagamentos | Financeiro, regressões | 38% | A cobertura Financeiro inclui regressões explícitas para pagamento canónico de faturas `material`, `inscricao` e `mensalidade` sem nº de recibo, garante que a Emissão Fiscal continua a exigir `external_document_number` no `mark-issued` e tem agora um contract test de frontend/source para garantir que a tab Faturas usa apenas métodos ativos, exige linha bancária quando aplicável e não recai no fluxo legado de movimentos. Continua insuficiente face ao tamanho da aplicação. |
