...
| Comunicação | 70% | H6a está em produção sobre campanhas/entregas existentes: dispatcher agendado, retries com backoff, leases, idempotência em três níveis, attempts, provider IDs e audit produtivo. Produção mantém a queue persistente `database`; o adapter Redis está configurado, sem cutover implícito. Seguem o cutover das automações ainda síncronas, adapters/webhooks reais e futura integração Redes. |
...
Próximo passo: H6b deve mover as automações ainda síncronas para esta outbox sem alterar preferências ou origens. Em paralelo, a campanha legacy agendada/vencida deve ser revista explicitamente; o sistema não a enviará sozinho. Um eventual cutover da queue produtiva `database` para Redis só deve ocorrer depois de validar a configuração efetiva do Supervisor. H3, H4, H5, H2.5, stock por variante e Família/EE estão estruturalmente fechados. A fila fiscal produtiva e a ação operacional Cloudflare R2 permanecem pendências operacionais separadas.
