...
|---|---|---|---|---|
| 2026-08-30 | Rotas / Loja | H2.5k extrai as 9 rotas administrativas de leitura da Loja para um décimo módulo controlado, preservando gate, prefixo nominal e adicionando gate explícito para 28 módulos carregados. | `routes/web_store_admin.php`; `WebRouteTopologyAuditTest`; `WebRouteTopologyWorkflowContractTest`; `.github/workflows/ci.yml` | Em validação; equivalência textual confirmada e `routes/web.php` reduzido para 229 linhas, 85 declarações diretas e 20 imports. Aguarda CI, merge, deploy e artifact; H2.5l fica reservado para Patrocínios. |
