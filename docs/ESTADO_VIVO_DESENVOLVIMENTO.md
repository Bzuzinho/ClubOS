...
| Eventos | 76% | Lifecycle, recorrência, convocatórias e integrações corrigidos. O formulário de criação/edição apresenta validação persistente em português, foco no primeiro campo inválido, grelha mobile responsiva e notificações globais efetivamente montadas; os campos de recorrência são excluídos de eventos simples e o payload completo do browser está coberto por regressão. O bugfix está integrado e deployado; permanecem a remoção de estruturas antigas, contract tests Eventos ↔ Desportivo e E2E das restantes operações críticas. |
