...
final class SportsConfigurationController extends Controller
{
    ...
    private function rules(string $catalog, bool $partial = false): array
    {
        ...
        return match ($catalog) {
            'athlete_statuses' => [
                ...$common,
                'nome_en' => ['nullable', 'string', 'max:120'],
                'descricao' => ['nullable', 'string', 'max:2000'],
                'cor' => ['nullable', 'regex:/^#[0-9A-Fa-f]{6}$/'],
                'counts_as_present' => ['sometimes', 'boolean'],
                'requires_reason' => ['sometimes', 'boolean'],
                'allows_training' => ['sometimes', 'boolean'],
                'allows_competition' => ['sometimes', 'boolean'],
            ],
            'training_types' => [
                ...$common,
                'nome_en' => ['nullable', 'string', 'max:120'],
                ...
                'is_high_intensity' => ['sometimes', 'boolean'],
            ],
            'training_zones' => [
                ...$common,
                'descricao' => ['nullable', 'string', 'max:2000'],
                'percentagem_min' => ['nullable', 'integer', 'min:0', 'max:200'],
                'percentagem_max' => ['nullable', 'integer', 'min:0', 'max:200', 'gte:percentagem_min'],
                'cor' => ['nullable', 'regex:/^#[0-9A-Fa-f]{6}$/'],
                'is_recovery' => ['sometimes', 'boolean'],
                'is_high_intensity' => ['sometimes', 'boolean'],
            ],
            'absence_reasons' => [
                ...
                'nome_en' => ['nullable', 'string', 'max:120'],
                'descricao' => ['nullable', 'string', 'max:2000'],
                'requer_justificacao' => ['sometimes', 'boolean'],
                'health_related' => ['sometimes', 'boolean'],
            ],
            'pool_types' => [
                ...$common,
                'comprimento_m' => ['nullable', 'integer', 'min:1', 'max:10000'],
                'is_open_water' => ['sometimes', 'boolean'],
            ],
            'race_types' => [
                ...$common,
                'distancia' => [$required, 'integer', 'min:1', 'max:100000'],
                'unidade' => [$required, Rule::in(['m', 'km'])],
                'modalidade' => [$required, 'string', 'max:80'],
            ],
            'limitation_types' => [
                ...$common,
                'descricao' => ['nullable', 'string', 'max:2000'],
                'instrucao_padrao' => ['nullable', 'string', 'max:3000'],
                'allows_training' => ['sometimes', 'boolean'],
                'allows_competition' => ['sometimes', 'boolean'],
                'requires_end_date' => ['sometimes', 'boolean'],
            ],
            default => throw \Illuminate\Validation\ValidationException::withMessages([
                'catalog' => 'Catálogo de configuração desportiva inválido.',
            ]),
        };
    }
}
