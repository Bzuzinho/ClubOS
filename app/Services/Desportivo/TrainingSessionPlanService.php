...
final class TrainingSessionPlanService
{
    ...
    /**
     * Atualiza apenas as sessões futuras explicitamente selecionadas.
     * Nenhuma sessão passada/concluída é modificada silenciosamente.
     *
     * @param array<int,string> $sessionIds
     * @return array<int,Training>
     */
    public function updateSelectedFutureSessions(
        TrainingPlanVersion $fromVersion,
        TrainingPlanVersion $toVersion,
        array $sessionIds,
        User $actor,
    ): array {
        $this->assertVersionTenant($fromVersion);
        $this->assertVersionTenant($toVersion);

        if ((string) $fromVersion->training_plan_id !== (string) $toVersion->training_plan_id) {
            throw ValidationException::withMessages([
                'training_plan_version_id' => 'As versões têm de pertencer ao mesmo plano de treino.',
            ]);
        }
        ...
        return DB::transaction(function () use ($ids, $fromVersion, $toVersion, $actor): array {
            $sessions = Training::query()
                ...
                ->lockForUpdate()
                ->get()
                ->keyBy(fn (Training $training): string => (string) $training->id);

            if ($sessions->count() !== $ids->count()) {
                throw ValidationException::withMessages([
                    'training_ids' => 'Uma ou mais sessões selecionadas já não existem.',
                ]);
            }
            ...
            foreach ($ids as $id) {
                ...
                if ($session->isCompleted()) {
                    throw ValidationException::withMessages([
                        'training_ids' => "A sessão {$session->numero_treino} já está concluída e não pode receber outra versão.",
                    ]);
                }

                if ($session->data === null || $session->data->startOfDay()->lt($today)) {
                    throw ValidationException::withMessages([
                        'training_ids' => "A sessão {$session->numero_treino} não é futura e não pode ser atualizada em lote.",
                    ]);
                }

                if ((string) $session->training_plan_version_id !== (string) $fromVersion->id) {
                    throw ValidationException::withMessages([
                        'training_ids' => "A sessão {$session->numero_treino} não utiliza a versão de origem selecionada.",
                    ]);
                }
    ...
    {
        $this->assertSessionTenant($session);
        $this->assertVersionTenant($version);

        if ($session->isCompleted()) {
            throw ValidationException::withMessages([
                'training_plan_version_id' => 'Uma sessão concluída não pode ser alterada por uma revisão do plano.',
            ]);
        }

        $version->loadMissing('series');
        ...
        foreach ($version->series as $line) {
            TrainingSeries::query()->create([
                ...
                'ordem' => $line->ordem,
                'descricao_texto' => $line->exercicio ?? '',
                'distancia_total_m' => $line->distancia_total_m ?? 0,
                ...
                'training_plan_series_id' => $line->id,
                'source' => 'plan_version',
                'bloco' => $line->bloco,
        ...
        $session->forceFill([
            'training_plan_version_id' => $version->id,
            'tipo_treino' => $version->tipo_treino ?: $session->tipo_treino,
            'volume_planeado_m' => $version->volume_planeado_m,
            'descricao_treino' => $version->descricao_treino,
            'notas_gerais' => $version->notas_gerais,
            'instrucao' => $version->instrucao,
            'plan_applied_at' => now(),
            'plan_applied_by' => $actor->id,
        ])->save();
    ...
    {
        if ((string) $version->club_id !== $this->clubContext->id()) {
            throw ValidationException::withMessages([
