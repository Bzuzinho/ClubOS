...
final class TrainingSessionGroupService
{
    ...
    public function replace(Training $session, array $assignments, ?User $actor = null): Collection
    {
        ...
        if ($session->isCompleted()) throw ValidationException::withMessages(['training_groups' => 'A composição planeada de uma sessão concluída não pode ser alterada.']);
        if ($session->isCancelled()) throw ValidationException::withMessages(['training_groups' => 'A composição planeada de uma sessão cancelada não pode ser alterada.']);
        ...
        if ($groupIds->count() !== $groupIds->unique()->count()) throw ValidationException::withMessages(['training_groups' => 'O mesmo grupo de treino não pode ser repetido na mesma sessão.']);
    ...
    private function createAssignment(Training $session, array $row, int $index, ?User $actor): TrainingSessionGroup
    {
        ...
        if ($group === null || ! $group->active) throw ValidationException::withMessages(['training_groups' => 'Existe pelo menos um grupo de treino inválido ou inativo.']);
        ...
        if ($planVersionId !== '') {
            ...
            if ($planVersion === null) throw ValidationException::withMessages(['training_groups' => 'Existe pelo menos uma versão de plano inválida para o clube ativo.']);
        }
        ...
        $lanes = $laneIds->isEmpty() ? collect() : SportsPoolLane::query()->with('pool.venue')->where('club_id', $clubId)->where('active', true)->whereIn('id', $laneIds->all())->get()->keyBy(fn (SportsPoolLane $lane): string => (string) $lane->id);
        if ($lanes->count() !== $laneIds->count()) throw ValidationException::withMessages(['training_groups' => 'Existe pelo menos uma pista canónica inválida ou inativa.']);

        $poolIds = $lanes->pluck('sports_pool_id')->map('strval')->unique()->values();
        if ($poolIds->count() > 1) throw ValidationException::withMessages(['training_groups' => 'As pistas de uma sessão têm de pertencer à mesma piscina/área.']);

        if ($poolIds->isNotEmpty()) {
            $firstLane = $lanes->first(); $pool = $firstLane->pool; $venue = $pool?->venue;
            if (! $pool || ! $venue || ! $pool->active || ! $venue->active) throw ValidationException::withMessages(['training_groups' => 'A piscina/área ou o local selecionado está inativo.']);
            if ($session->sports_pool_id && (string) $session->sports_pool_id !== (string) $pool->id) throw ValidationException::withMessages(['training_groups' => 'As pistas não pertencem à piscina/área definida para a sessão.']);
            if ($session->sports_venue_id && (string) $session->sports_venue_id !== (string) $venue->id) throw ValidationException::withMessages(['training_groups' => 'As pistas não pertencem ao local definido para a sessão.']);
            $session->forceFill(['sports_venue_id' => $venue->id,'sports_pool_id' => $pool->id,'local' => filled($session->local) ? $session->local : $venue->name])->save();
        }

        $assignment = TrainingSessionGroup::query()->create([
            'club_id' => $clubId,'training_id' => $session->id,'training_group_id' => $group->id,
            'training_plan_version_id' => $planVersion?->id,'instruction' => $instruction,
            'sort_order' => (int) ($row['sort_order'] ?? $index),'created_by' => $actor?->id,
        ]);
