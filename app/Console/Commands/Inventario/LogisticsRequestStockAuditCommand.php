...
use App\Services\Inventario\LogisticsRequestStockAuditService;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\File;

final class LogisticsRequestStockAuditCommand extends Command
{
    protected $signature = 'inventory:audit-logistics-request-stock
        ...
        {--request= : Filtrar por requisicao logistica}
    ...
    protected $description = 'Auditoria read-only de reservas, saidas e entregas de stock em requisicoes logisticas';

    public function __construct(
        private readonly LogisticsRequestStockAuditService $auditService,
    ) {
        parent::__construct();
    }
    ...
    {
        $this->info('Logistics request stock audit');
        $this->table(
            ['Metric', 'Value'],
        ...
        $this->table(
            ['Severity', 'Code', 'Request', 'Material', 'Qty Request', 'Reserved Net', 'Physical Net', 'Movements', 'Actionable', 'Recommendation'],
            collect($payload['findings'] ?? [])
                ->map(static fn (array $finding): array => [
                    ...
                    (string) ($finding['quantity_request'] ?? ''),
                    (string) ($finding['reserved_net'] ?? ''),
