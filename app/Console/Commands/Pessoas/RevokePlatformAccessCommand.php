...
final class RevokePlatformAccessCommand extends Command
{
    protected $signature = 'people:revoke-platform-access
    ...
    protected $description = 'Revoga acesso explicito ativo a plataforma';
    ...
    public function handle(): int
    {
        ...
        if (! $dryRun) {
            $this->platformAccessService->revokePlatformAccess($user, $actor, $this->notes());
            $applied = true;
        }
    ...
    private function payload(User $user, array $before, array $after, bool $dryRun, bool $applied): array
    {
        return [
            ...
            'new_platform_access_enabled' => $dryRun ? false : (bool) ($after['platform_access_enabled'] ?? false),
            'dry_run' => $dryRun,
            'applied' => $applied,
            'notes' => $this->notes(),
        ];
